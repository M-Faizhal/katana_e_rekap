    ...
    return `
        <tr class="hover:bg-gray-50 ${hasClientMapping ? 'bg-blue-50' : ''}">
            ...
            <td class="px-2 py-3 bg-yellow-100">
                <input type="number" value="${item.harga_diskon > 0 ? item.harga_diskon : ''}" onchange="updateValue(${index}, 'harga_diskon', this.value)" class="no-spin text-right w-20 font-semibold" placeholder="0" title="INPUT: Masukkan harga akhir setelah diskon" ${!canEdit ? 'readonly' : ''}>
            </td>
            ...
            <td class="px-2 py-3 bg-yellow-100">
                <input type="number" value="${item.harga_yang_diharapkan > 0 ? item.harga_yang_diharapkan : ''}" onchange="updateValue(${index}, 'harga_yang_diharapkan', this.value)" class="no-spin text-right w-20 font-semibold" placeholder="0" title="INPUT: Masukkan harga yang diharapkan" ${!canEdit ? 'readonly' : ''}>
            </td>
            <td class="px-2 py-3 bg-gray-50 text-xs">
                <span class="text-gray-600 ${(item.persen_kenaikan || 0) >= 0 ? 'text-green-700' : 'text-red-700'}" title="CALCULATED: (((Harga Yang Diharapkan × QTY) - (Total Harga hpp + Nilai PPH + Nilai PPN)) / Total Harga hpp) × 100">${formatPercent(item.persen_kenaikan || 0)}</span>
            </td>
            ...
            <td class="px-2 py-3">
                <input type="number" value="${item.nilai_sp > 0 ? item.nilai_sp : ''}" onchange="updateValue(${index}, 'nilai_sp', this.value)" class="no-spin text-right w-20" placeholder="0" ${!canEdit ? 'readonly' : ''}>
            </td>
            ...
            <td class="px-2 py-3">
                <input type="number" value="${item.ongkir > 0 ? item.ongkir : ''}" onchange="updateValue(${index}, 'ongkir', this.value)" class="no-spin text-right w-16" placeholder="0" ${!canEdit ? 'readonly' : ''}>
            </td>
            <td class="px-2 py-3">
                <input type="number" value="${item.omzet_dinas_percent > 0 ? item.omzet_dinas_percent : ''}" onchange="updateValue(${index}, 'omzet_dinas_percent', this.value)" class="no-spin text-right w-12" step="0.1" placeholder="0" ${!canEdit ? 'readonly' : ''}>
            </td>
            <td class="px-2 py-3 bg-red-50 text-xs">
                <span>${formatRupiah(item.omzet_nilai_dinas || 0)}</span>
            </td>
            <td class="px-2 py-3">
                <input type="number" value="${item.bendera_percent > 0 ? item.bendera_percent : ''}" onchange="updateValue(${index}, 'bendera_percent', this.value)" class="no-spin text-right w-12" step="0.1" placeholder="0" ${!canEdit ? 'readonly' : ''}>
            </td>
            <td class="px-2 py-3 bg-red-50 text-xs">
                <span>${formatRupiah(item.gross_nilai_bendera || 0)}</span>
            </td>
            <td class="px-2 py-3">
                <input type="number" value="${item.bank_cost_percent > 0 ? item.bank_cost_percent : ''}" onchange="updateValue(${index}, 'bank_cost_percent', this.value)" class="no-spin text-right w-12" step="0.1" placeholder="0" ${!canEdit ? 'readonly' : ''}>
            </td>
            <td class="px-2 py-3 bg-red-50 text-xs">
                <span>${formatRupiah(item.gross_nilai_bank_cost || 0)}</span>
            </td>
            <td class="px-2 py-3">
                <input type="number" value="${item.biaya_ops_percent > 0 ? item.biaya_ops_percent : ''}" onchange="updateValue(${index}, 'biaya_ops_percent', this.value)" class="no-spin text-right w-12" step="0.1" placeholder="0" ${!canEdit ? 'readonly' : ''}>
            </td>
            <td class="px-2 py-3 bg-red-50 text-xs">
...
function updateValue(index, field, value) {
    // Handle empty values - convert empty string to 0 for calculations
